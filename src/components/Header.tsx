...
export const Header: React.FC = () => {
  ...
  const navLinks = [
    ...
    { name: 'SANS Compliance', view: 'compliance-dashboard' },
  ...
  return (
    <>
      ...
      <div className="bg-[#071322] text-slate-300 text-xs font-mono py-2 px-4 sm:px-6 border-b border-slate-800 shadow-xs">
        <div className="max-w-7xl mx-auto flex flex-col xl:flex-row items-center justify-between gap-2.5">
          ...
          <div className="flex items-center flex-wrap justify-center gap-2.5 sm:gap-4 text-[11px]">
            ...
            {/* Night-Time Inspection Mode Theme Switcher */}
            ...
            <div className="relative">
              <button
                ...
                className="inline-flex items-center gap-1 bg-slate-800 hover:bg-slate-700 text-white px-2 py-0.5 rounded-sm text-[10px] font-bold tracking-wider uppercase transition-colors cursor-pointer border border-slate-700 active:scale-95"
                ...
                {isRoleSwitcherOpen && (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full text-left px-3 py-2 hover:bg-slate-800 text-slate-200 flex items-center justify-between cursor-pointer transition-colors"
                    >
                      <div>
      ...
      {/* Main Sticky Header */}
      <header className="sticky top-0 z-40 bg-white dark:bg-[#0A192F] border-b border-gray-100 dark:border-slate-800 shadow-sm transition-colors duration-150">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          >
            <BrandLogo size="md" variant={theme === 'dark' ? 'white' : 'dark'} />
          </div>

          {/* Desktop Navigation Links */}
          <nav className="hidden lg:flex items-center gap-6 text-[11px] font-bold uppercase tracking-widest text-gray-500 dark:text-slate-300">
            {navLinks.map((link) => {
              ...
              return (
                <button
                  ...
                  className={`relative py-2 transition-colors cursor-pointer ${
                    isActive
                      ? 'text-[#0A192F] dark:text-white font-extrabold'
                      : 'hover:text-[#0A192F] dark:hover:text-white'
                  }`}
                >
                  <span>{link.name}</span>
                  {isActive && (
          ...
          <div className="hidden sm:flex items-center gap-3">
            {/* Night-Time Site Inspection Mode Pill Switcher */}
            ...
            <motion.button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-[#CC0000] hover:bg-red-50 dark:hover:bg-red-950/40 border border-[#CC0000] px-3.5 py-2 rounded-sm transition-colors cursor-pointer"
            >
              <AlertTriangle className="w-3.5 h-3.5 text-[#CC0000] animate-pulse" />
            ...
            <motion.button
              ...
              className="inline-flex items-center gap-2 bg-[#0A192F] dark:bg-[#112240] hover:bg-slate-800 dark:hover:bg-[#1E3A5F] border border-transparent dark:border-[#1E3A5F] active:bg-slate-900 text-white px-5 py-2.5 rounded-sm text-xs font-bold uppercase tracking-widest shadow-md transition-colors cursor-pointer"
            >
            ...
            {currentUser ? (
              <div className="relative">
                <button
                  ...
                  className="flex items-center gap-2 bg-gray-50 dark:bg-[#0E2442] hover:bg-gray-100 dark:hover:bg-[#15345E] text-[#0A192F] dark:text-slate-100 px-3 py-2 rounded-sm text-xs font-bold border border-gray-200 dark:border-[#1E3A5F] transition-colors cursor-pointer active:scale-95"
                >
                  <div className="w-6 h-6 rounded-sm bg-[#0A192F] dark:bg-[#CC0000] text-white flex items-center justify-center text-xs font-black">
                ...
                <AnimatePresence>
                  {isUserMenuOpen && (
                    <motion.div
                      ...
                      transition={{ duration: 0.15, ease: 'easeOut' }}
                      className="absolute right-0 mt-2 w-64 bg-white dark:bg-[#0A192F] border border-gray-200 dark:border-slate-700 rounded-sm shadow-xl py-2 z-50 text-[#0A192F] dark:text-slate-100"
                      onClick={() => setIsUserMenuOpen(false)}
                    >
                      <div className="px-4 py-2.5 border-b border-gray-100 dark:border-slate-800">
                        <p className="font-bold text-sm text-[#0A192F] dark:text-white">{currentUser.fullName}</p>
                        <p className="text-xs text-gray-500 dark:text-slate-400 truncate">{currentUser.email}</p>
                        <span className="inline-block mt-1 text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-sm bg-gray-100 dark:bg-slate-800 text-[#0A192F] dark:text-slate-300">
                      ...
                      <button
                        ...
                        className="w-full text-left px-4 py-2.5 text-xs font-bold uppercase tracking-wider hover:bg-gray-50 dark:hover:bg-slate-800/80 flex items-center gap-2 text-[#0A192F] dark:text-slate-200 cursor-pointer"
                      >
                      ...
                      <button
                        onClick={() => handleNavClick('compliance-dashboard')}
                        className="w-full text-left px-4 py-2.5 text-xs font-bold uppercase tracking-wider hover:bg-gray-50 dark:hover:bg-slate-800/80 flex items-center gap-2 text-amber-500 cursor-pointer"
                      >
                        <ShieldCheck className="w-4 h-4 text-amber-500" />
                        <span>SANS Compliance Dashboard</span>
                      </button>

                      {(currentUser.role === 'admin' || currentUser.role === 'superadmin' || currentUser.role === 'staff') && (
                        <button
                          onClick={() => handleNavClick('admin-portal')}
                          className="w-full text-left px-4 py-2.5 text-xs font-bold uppercase tracking-wider hover:bg-gray-50 dark:hover:bg-slate-800/80 flex items-center gap-2 text-[#CC0000] cursor-pointer"
                        >
                      ...
                      <button
                        onClick={logout}
                        className="w-full text-left px-4 py-2 text-xs font-bold uppercase tracking-wider hover:bg-red-50 dark:hover:bg-red-950/40 text-[#CC0000] flex items-center gap-2 cursor-pointer"
                      >
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-2 text-xs font-bold uppercase tracking-widest text-[#0A192F] dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-800 rounded-sm transition-colors"
                >
                  Log In
                </button>
                <button
                  onClick={() => handleNavClick('auth')}
                  className="px-3.5 py-2 text-xs font-bold uppercase tracking-widest bg-[#0A192F] dark:bg-[#112240] text-white hover:bg-slate-800 dark:hover:bg-[#1E3A5F] border border-transparent dark:border-[#1E3A5F] rounded-sm shadow-sm transition-colors"
                >
                  Register
                </button>
              </div>
          ...
          <div className="flex items-center gap-2 lg:hidden">
            ...
            <button
              ...
              className="p-2.5 rounded-sm text-[#0A192F] dark:text-white hover:bg-gray-100 dark:hover:bg-slate-800 transition-colors"
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className="lg:hidden bg-white dark:bg-[#0A192F] border-b border-gray-200 dark:border-slate-800 px-4 pt-3 pb-6 shadow-xl overflow-hidden"
            >
              {/* Mobile Night Inspection Mode Toggle */}
                ...
                {navLinks.map((link) => {
                  ...
                  return (
                    <button
                      key={link.view}
                      onClick={() => handleNavClick(link.view)}
                      className={`w-full text-left px-4 py-3 rounded-sm text-xs font-bold uppercase tracking-wider transition-colors flex items-center justify-between cursor-pointer ${
                        isActive
                          ? 'bg-gray-100 dark:bg-slate-800 text-[#0A192F] dark:text-white border-l-4 border-[#CC0000]'
              ...
              <div className="mt-3 p-3 bg-slate-900 text-white rounded-md border border-slate-800 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 pt-1 font-mono text-xs">
                  <a
                    href={`tel:${COMPANY_DETAILS.telephone.replace(/\s+/g, '')}`}
                    className="inline-flex items-center justify-center gap-1.5 bg-[#CC0000] hover:bg-red-700 text-white py-2 px-2 rounded-sm font-bold text-center active:scale-95 transition-transform"
                  >
                  ...
                  <a
                    href={`mailto:${COMPANY_DETAILS.email}`}
                    className="inline-flex items-center justify-center gap-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 py-2 px-2 rounded-sm font-medium border border-slate-700 text-center text-[11px] active:scale-95 transition-transform"
                  >
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 space-y-2">
                <button
                  onClick={() => openRequestService()}
                  className="w-full py-3 bg-[#0A192F] hover:bg-slate-800 text-white rounded-sm font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 shadow-md active:scale-98 transition-all cursor-pointer"
                >
                  <FilePlus className="w-4 h-4 text-[#CC0000]" />
                ...
                <button
                  onClick={openEmergencyFault}
                  className="w-full py-2.5 bg-white text-[#CC0000] hover:bg-red-50 border-2 border-[#CC0000] rounded-sm font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 active:scale-98 transition-all cursor-pointer"
                >
                ...
                <div className="pt-2">
                  {currentUser ? (
                    <div className="space-y-1">
                      <button
                        onClick={() => handleNavClick('customer-portal')}
                        className="w-full text-left px-4 py-2.5 text-xs bg-gray-100 dark:bg-slate-800 rounded-sm font-bold uppercase tracking-wider text-[#0A192F] dark:text-slate-100 flex items-center gap-2 cursor-pointer"
                      >
                      ...
                      {(currentUser.role === 'admin' || currentUser.role === 'superadmin' || currentUser.role === 'staff') && (
                        <button
                          onClick={() => handleNavClick('admin-portal')}
                          className="w-full text-left px-4 py-2.5 text-xs bg-[#0A192F] text-white rounded-sm font-bold uppercase tracking-wider flex items-center gap-2 cursor-pointer"
                        >
                          <Shield className="w-4 h-4 text-[#CC0000]" />
                          <span>Operations & Django CMS</span>
                        </button>
                      ...
                      <button
                        onClick={logout}
                        className="w-full text-left px-4 py-2 text-xs font-bold text-[#CC0000] cursor-pointer"
                      >
                        Sign Out
                      </button>
                    </div>
                    ...
                    <button
                      onClick={() => handleNavClick('auth')}
                      className="w-full py-2.5 border-2 border-[#0A192F] dark:border-slate-600 text-[#0A192F] dark:text-slate-100 font-bold text-xs uppercase tracking-widest rounded-sm hover:bg-gray-50 dark:hover:bg-slate-800 cursor-pointer"
                    >
